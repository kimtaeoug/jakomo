...
class STHistoryController extends GetxController {
  ...
  //RxList<DateTime> dateList = <DateTime>[].obs;
  List<STModel> getData(Map<DateTime, List<STModel>> dataMap, DateTime date) {
  ...
  //날짜(일) 별로 parsing
  Map<DateTime, List<STModel>> parsingData(List<STModel> dataList) {
    Map<DateTime, List<STModel>> parsedData = {};
    int idx = 0;
  ...
  double averageHR(List<STModel> dataList) {
    int idx = 0;
    double result = 0;
    for (STModel data in dataList) {
      result += data.stress;
