...
class WEHistoryController extends GetxController {
  ...
  //RxList<DateTime> dateList = <DateTime>[].obs;
  List<WEModel> getData(Map<DateTime, List<WEModel>> dataMap, DateTime date) {
  ...
  //날짜(일) 별로 parsing
  Map<DateTime, List<WEModel>> parsingData(List<WEModel> dataList) {
    Map<DateTime, List<WEModel>> parsedData = {};
    int idx = 0;
  ...
  double averageHR(List<WEModel> dataList) {
    int idx = 0;
    double result = 0;
    for (WEModel data in dataList) {
      result += data.weight;
  ...
  FlSpot convertFlSpot(int dataIdx, List<WEModel> dataList){
    int idx = 0;
    double result = 0;
    for(WEModel data in dataList){
      result += data.weight;
    ...
    if(result == 0){
      return FlSpot(dataIdx.toDouble(), 0);
    }else{
      return FlSpot(dataIdx.toDouble(), result/idx);
    }
  }
}
